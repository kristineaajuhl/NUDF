...
class Trainer(object):
    ...
    def compute_loss(self,batch):
        ...
        # debug_path = os.path.dirname(__file__) + '/../debug/'+current_time + '.npz'
        # np.savez(debug_path,p=p.cpu(),d=occ.cpu(),x=inputs.cpu())
        ...
        # UDF-wise: 
        clamp_dist = 0.1
        pred_sdf = self.model(p,inputs)
        sdf_gt = torch.clamp(occ,-clamp_dist,clamp_dist)
        pred_sdf = torch.clamp(pred_sdf,-clamp_dist,clamp_dist)
        loss_i = F.l1_loss(sdf_gt,pred_sdf,reduction='none')#*1/(sdf_gt+1e-3)
